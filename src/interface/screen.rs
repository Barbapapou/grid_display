        ...
        while let Some(action) = self.action_queue.pop_front() {
            match action {
                UiAction::AddUiText(text, pos, size) => {
                    let ui_text = Box::new(UiText::new(text, pos, size));
                    self.add_element(ui_text);
                }
            }
        }
